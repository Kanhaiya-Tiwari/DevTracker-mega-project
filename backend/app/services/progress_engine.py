...
def completion_probability(
    ...
    avg_hours = sum(entry["hours"] for entry in recent_logs) / len(recent_logs)
    consistency = sum(1 for entry in recent_logs if entry["hours"] >= daily_target * 0.7) / len(recent_logs)
...
def consistency_score(logs: List[dict], skill_id: str, window_days: int = 14) -> int:
    ...
    relevant = [entry for entry in logs if entry["skill_id"] == skill_id and entry["log_date"] >= cutoff]
...
def detect_burnout(logs: List[dict], skill_id: str) -> bool:
    recent = [entry for entry in logs if entry["skill_id"] == skill_id][-7:]
    if len(recent) < 4:
        return False
    avg_recent = sum(entry["hours"] for entry in recent[-3:]) / 3
    avg_prior  = sum(entry["hours"] for entry in recent[:4])  / 4
    return avg_prior > 0 and (avg_recent / avg_prior) < 0.5

def detect_peak_hour(logs: List[dict]) -> int | None:
    dist: dict[int, float] = {}
    for log_entry in logs:
        h = log_entry.get("hour_of_day")
        if h is not None:
            dist[h] = dist.get(h, 0) + log_entry["hours"]
